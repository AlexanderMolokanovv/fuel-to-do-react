...
# Функция для визуализации процесса импутации для конкретной характеристики
def visualize_imputation(field_index):
    field_name = characteristic_fields[field_index]
    known_values = original_data_np[~nan_mask[:, field_index], field_index]
    imputed_values = data_tensor.numpy()[nan_mask[:, field_index], field_index]
    
    fig, ax = plt.subplots(figsize=(8, 6))
    if len(known_values) > 0:
        ax.hist(known_values, bins=20, alpha=0.5, label='Known Values', color='blue')
    if len(imputed_values) > 0:
        for val in imputed_values:
            ax.axvline(val, color='red', linestyle='dashed', linewidth=1, label='Imputed Value' if 'Imputed Value' not in [l.get_label() for l in ax.lines] else "")
        ax.hist(imputed_values, bins=20, alpha=0.5, label='Imputed Values (Histogram)', color='orange')
    ax.set_title(f'Distribution of {field_name} with Imputed Values')
    ax.set_xlabel('Value')
...
# Функция для отображения графиков импутации
def show_imputation_graphs():
    try:
        print("Attempting to display imputation graphs window...")
        graph_window = tk.Tk()
        graph_window.title("Imputation Graphs")
        
        # Проверяем, есть ли пропуски в данных
        if not np.any(nan_mask):
            tk.Label(graph_window, text="No missing values found, no imputation graphs to display.").pack(pady=10)
            tk.Button(graph_window, text="Continue to Reconstructed Data", 
                     command=lambda: [graph_window.destroy(), show_reconstructed_data()]).pack(pady=10)
            print("No missing values, skipping graphs...")
        ...
        for idx, field in enumerate(characteristic_fields):
            if np.any(nan_mask[:, idx]):  # Только для характеристик с пропусками
                fig = visualize_imputation(idx)
                frame = ttk.Frame(notebook)
                canvas = FigureCanvasTkAgg(fig, master=frame)
                canvas.draw()
                canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
                notebook.add(frame, text=field)
        ...
        print("Imputation graphs window created, starting mainloop...")
        graph_window.mainloop()
    except Exception as e:
        print(f"Ошибка при отображении графиков импутации: {traceback.format_exc()}")
        messagebox.showerror("GUI Error", f"Failed to display imputation graphs: {e}\nProceeding to reconstructed data...")
        show_reconstructed_data()  # Fallback to reconstructed data
...
# Функция для построения графика Парето
def show_pareto_graph(best_mixtures, user_target):
    try:
        print("Attempting to display Pareto graph window...")
        pareto_window = tk.Tk()
        pareto_window.title("Pareto Front")
        
        if not best_mixtures:
            tk.Label(pareto_window, text="No valid mixtures found for Pareto front.").pack(pady=10)
            tk.Button(pareto_window, text="Continue to Results", 
                     command=lambda: [pareto_window.destroy(), show_results(best_mixtures)]).pack(pady=10)
            print("No mixtures for Pareto graph, skipping...")
            pareto_window.mainloop()
            return

        # Вычисляем значения для графика Парето
        losses = [mixture[2] for mixture in best_mixtures]  # Значение функции потерь
        deviations = []
        for mixture in best_mixtures:
            mixture_data = np.array(mixture[3])  # Значения характеристик смеси
            target = user_target.numpy()
            deviation = np.sum(np.abs(mixture_data - target))  # Суммарное отклонение
            deviations.append(deviation)

        # Определяем фронт Парето
        pareto_points = []
        for i in range(len(best_mixtures)):
            is_pareto = True
            for j in range(len(best_mixtures)):
                if i != j and losses[j] <= losses[i] and deviations[j] <= deviations[i] and (losses[j] < losses[i] or deviations[j] < deviations[i]):
                    is_pareto = False
                    break
            if is_pareto:
        ...
        # Создаем график
        fig, ax = plt.subplots(figsize=(8, 6))
        ax.scatter(losses, deviations, c='blue', label='All Mixtures', alpha=0.6)
        if pareto_points:
            pareto_losses = [losses[i] for i in pareto_points]
            pareto_deviations = [deviations[i] for i in pareto_points]
            ax.scatter(pareto_losses, pareto_deviations, c='red', label='Pareto Front', s=100, marker='*')
        
        ax.set_xlabel('Loss (MSE)')
        ax.set_ylabel('Total Absolute Deviation from Target')
        ax.set_title('Pareto Front of Optimal Mixtures')
        ax.legend()
        ax.grid(True)
        plt.tight_layout()

        # Интеграция графика в Tkinter
        canvas = FigureCanvasTkAgg(fig, master=pareto_window)
        canvas.draw()
        canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
        ...
        print("Pareto graph window created, starting mainloop...")
        pareto_window.mainloop()
        plt.close(fig)  # Закрываем фигуру
    except Exception as e:
        print(f"Ошибка при отображении графика Парето: {traceback.format_exc()}")
        messagebox.showerror("GUI Error", f"Failed to display Pareto graph: {e}\nProceeding to results...")
        show_results(best_mixtures)  # Fallback to results

# Функция для отображения текстовых результатов оптимизации
def show_results(best_mixtures):
    try:
        if not best_mixtures:
            error_text = "No valid mixtures found.\n"
        ...
        print(f"Ошибка при отображении результатов: {traceback.format_exc()}")
        messagebox.showerror("GUI Error", f"Failed to display results: {e}")
        create_main_gui()
...
def optimize_mixture():
    try:
        ...
        global user_target  # Сохраняем user_target для использования в show_pareto_graph
        ...
        # Показываем график Парето перед текстовыми результатами
        show_pareto_graph(best_mixtures, user_target)
...
if __name__ == "__main__":
    try:
        ...
        show_imputation_graphs()  # Показываем графики импутации
    except Exception as e:
